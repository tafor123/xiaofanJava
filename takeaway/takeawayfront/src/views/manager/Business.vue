<template>
  <div>
    <div class="search">
      ...
      <el-input placeholder="请输入姓名查询" style="width: 200px;margin-left: 4px" v-model="name"></el-input>
          ...
          <template v-slot = "scope">
            <div style="display: flex;align-items: center">
              <el-image style="width: 40px;height: 40px;border-radius: 50%;"
                        ...
                        :perview-src-list="[scope.row.avatar]"
              >
              </el-image>
            </div>
          </template>
          ...
          <template v-slot="scope">
            <div style="display: flex;align-items: center">
              <el-image style="width: 40px;height: 40px;border-radius: 50%;"
                        ...
                        :perview-src-list="[scope.row.license]"
              >
              </el-image>
            </div>
          </template>
      ...
      <el-form :model="form" label-width="100px" style="padding-right: 50px" :rules="rules" ref="formRef">
        ...
        <el-form-item label="类型" prop="type">
          <el-select style="width: 100%" v-model="form.type">
            <el-option v-for="item in ['奶茶饮品', '轻食简餐', '炸鸡汉堡', '特色美味']" :key="item"
                       :value="item"
                       :label="item"
            ></el-option>
          </el-select>
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(rows) {   // 当前选中的所有的行数据
      this.ids = rows.map(v => v.id) //数组转对象
    },
    delBatch() {   // 批量删除
      if (!this.ids.length) {
